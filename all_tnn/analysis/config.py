...
class Config:
    ...
    def __init__(self, base_path = '/share/klab/datasets/',   analysis_dir = '/share/klab/datasets/TNN_paper_save_dir/All-TNN_share-test-varun/'):
            # -------------------------------------------------------------------
            # -----------------------  Dataset paths setup  ----------------------
            # -------------------------------------------------------------------
            ...
            self.ANALYSIS_DIR = analysis_dir  # Where the analysis related data are stored
            self.STF_PATH =self.ANALYSIS_DIR + '/datasets/category_selectivity_stimuli.h5'  # Scenes, tools, faces datasets for category selectivity analysis

            # -------------------------------------------------------------------
            # -----------------------  Model paths setup  ------------------------
            # -------------------------------------------------------------------
            self.BASE_MODEL_DIR = self.ANALYSIS_DIR + 'shared_weights/'  # Base directory for all models 
            self.NEURAL_LEVEL_RESULT_DIR = self.ANALYSIS_DIR + 'neural_level_src/' + 'neural_analysis_results/'  # Neural-level results
            ...
            self.EARLY_STOPPING_FLAG = [False, True][1]  # Whether to use early stopping
            ...
            self.MODEL_NAMES = list(self.MODEL_NAME_PATH_DICT.keys())
            ...
            self.MODEL_NAME_PATH_DICT = {
                ...
                for model, subdir in self.MODEL_NAME_PATH_DICT.items() if subdir is not None
            }
            ...
            self.SUMMARY_of_NEURAL_LEVEL_ANALYSES = [False, True][1]   # Summarize neural-level analyses at the end
            ...
            self.SELECTIVITY_DATASETS = ['vgg_faces_test', 'scenes_test', 'tools_test']
            ...
            self.SIZE_FACTORS = ['relative_white_controlled_original_largest205_min20px_dataset_filtered_num500_grid_fixed_back255']
            ...
            # -------------------------------------------------------------------
            self.BEHAVIOUR_ANALYSIS_RESULT_DIR = self.BEHAVIOUR_RESULT_SOURCE_DIR + 'behaviour_analysis_results/'
            self.BEHAVIOUR_AGREEMENT_ANALYSIS_PATH = self.BEHAVIOUR_ANALYSIS_RESULT_DIR + "/behaviour_agreements.pdf"
            self.ADM_AGREEMENT_ANALYSIS_PATH = self.BEHAVIOUR_ANALYSIS_RESULT_DIR + "/adm_agreements.pdf"
