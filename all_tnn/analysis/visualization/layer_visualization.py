...
from all_tnn.analysis.util.analysis_help_funcs import (
    format_weights,
    mask_array_by_value,
)
from all_tnn.models.model_helper.tnn_helper_functions import (
    channels_to_sheet,
    compute_weights_cosine_distance,
    extract_weight_dims,
)
...
def visualize_layer(
        ...
        if "category_selectivities" in output_dict:
            ...
            for k in mean_maps.keys():
                if "out" not in k:
                    ...
                    dprime_maps[k] = (in_means - out_means) / (
                        (in_stds + out_stds) / 2 + 1e-10
                    )

            data_dict = plot_category_selectivity_maps(
                data_dict,
                dprime_maps,
                epoch,
                layer_i,
        ...
        if "grating_w_tuning_curves" in output_dict:
            ...
            for i_w, f in enumerate(config.WAVELENGTHS):
                ...
                data_dict[f"activations_w{config.WAVELENGTHS[i_w]}"] = {
                    "data": np.expand_dims(
                        hue, axis=0
                    ),  # add batch dims, since [b,h,w,c] is expected
                    "shape": "hsv",
                    "cm": plt.cm.hsv,
                }
    ...
    if config.GET_SPATIAL_LOSS and layer is not None:
        if "mean_cosdist_loss" not in data_dict.keys():
            ...
            if "conv_control" in model_name:
                ...
                (
                    row_col_weights,
                    row_col_weights_without_bias_norm,
                    row_col_weights_with_bias_norm,
                    kernel_weights,
                    bias_weights,
                ) = format_weights(
                    kernel_weights,
                    ...
                    in_channels,
                    out_channels,
                    conv_control_net=True,
                )
            else:
                # "standard" non-convolutional locally connected case
                n_row, n_col, in_channels, channel_dim = extract_weight_dims(
        ...
        spatial_loss_path = os.makedirs(
            f"{analysis_dir}/spatial_loss_layer/ep{epoch:03}/{layer_i}",
            exist_ok=True,
        )
        mean_cosdist = compute_weights_cosine_distance(
            kernel_weights,
            bias_weights,
            kernel_size,
            circular=True,
...
def plot_category_selectivity_maps(
    ...
    fig, ax = plt.subplots()
    dprime_cmaps = [
        make_linear_cm_extend(217, 95, 2),
        make_linear_cm_extend(27, 158, 119),
        make_linear_cm_extend(117, 112, 179),
    ]
    dprime_cmaps_dict = {
        k: i for k, i in zip(dprime_maps.keys(), dprime_cmaps)
    }
    dprime_sheets = {
        k: channels_to_sheet(v, return_np=True) for k, v in dprime_maps.items()
    }
...
def plot_orientation_entropy_maps(
    plot_type,
    data,
    epoch,
    layer_i,
    ...
    if data.get("shape") == "hsv":
        ...
        data_grid = np.stack(
            [data_grid, np.ones(data_grid.shape), np.ones(data_grid.shape)],
            axis=-1,
        )
    ...
    plt.figure()
    plt.imshow(
        mask_array_by_value(data_grid, mask_value),
        norm=data.get("norm"),
        cmap=data.get("cm", plt.cm.viridis),
    )
    ax = plt.gca()
...
def visualize_model(
    model_layers, epoch, output_dict, hparams, get_spatial_loss=False
):
    """Iterate through the (Hypercolumn) layers and visualize corresponding data"""
    config = Config()
    if "conv_control" in hparams["model_name"]:
        hypercolumn_layers = [
            ldx for ldx in model_layers if ldx.__class__.__name__ == "Conv2D"
        ]
        ...
        hypercolumn_layers = [
            ldx
            for ldx in model_layers
            if ldx.__class__.__name__ == "Hypercolumn2D"
        ]
    ...
    with tqdm(total=len(hypercolumn_layers) + 1) as pbar:
        ...
        for i, layer in enumerate(hypercolumn_layers):
            ...
            if get_spatial_loss and config.GET_SPATIAL_LOSS:
...
def analyze_distance_vs_weight_similarity(
    layer_i, row_col_weights, hparams, PRE_RELU, layers_to_analyze=[5]
):
    """
    ...
    if layer_i not in layers_to_analyze:
        print(
            f"Skip layer {layer_i} for distance_vs_weight_similarity analysis"
        )
        return
    ...
    row_col_weights = backend.reshape(
        row_col_weights,
        shape=(
            n_rows * c_dim[0] * n_cols * c_dim[1],
    ...
    coordinates = (
        np.indices((n_rows * c_dim[0], n_cols * c_dim[1]))
        .transpose(1, 2, 0)
        .reshape((n_rows * c_dim[0] * n_cols * c_dim[1], 2))
    )
...
def plot_similarity_vs_distance(dist, corr, layer_i, hparams, PRE_RELU):
    ...
    ax.scatter(
        dist, corr, s=0.1, c="k", linewidths=0, alpha=0.5, rasterized=True
    )
    remove_spines(ax)
